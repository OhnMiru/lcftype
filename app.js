...
async function callTelegramApi(action, extra = {}) {
  ...
  if (error) {
    throw new Error(
      error.message || 'Ошибка Edge Function'
    );
  }
...
function openUsernameDialog(currentUsername) {
  return new Promise(resolve => {
    ...
    const save = overlay.querySelector(
      '#profileSaveBtn'
    );
 ...
 * ВАЖНО:
 *
 * Эти классы соответствуют существующему style.css:
 *
 * .feed-tools
 * .feed-search
 * .search-icon
  ...
  return `
    <div class="feed-tools chrome">
      ...
      <div class="feed-search">
        ...
        <button
          id="clearSearchBtn"
          class="search-clear${state.search ? ' visible' : ''}"
          type="button"
          aria-label="Очистить"
        >
          ×
      ...
      <button
        id="authorFilterBtn"
        class="feed-filter-btn${selectedCount ? ' active' : ''}"
        type="button"
      >
      ...
      >
        ...
        <div class="author-filter-header">
          ...
          <div>
            ...
            <div class="author-filter-subtitle">
              Можно выбрать нескольких авторов
            </div>
        ...
        <div class="author-filter-list">
          ...
          ${
            authors.length
              ? authors.map(name => `
                <label class="author-filter-option">
                  ...
                  <span class="author-filter-check"></span>
...
function updateFeedToolState() {
  ...
  if (filter) {
    ...
    let count =
      filter.querySelector('.filter-count');

    if (state.authorFilter.size) {
      if (!count) {
...
function renderAuthorFilterPanel() {
  ...
  bindAuthorFilterPanel();
}


function bindAuthorFilterPanel() {
  ...
  document
    .getElementById('doneAuthorFilter')
    ?.addEventListener(
      'click',
      () => {
        panel.hidden = true;
